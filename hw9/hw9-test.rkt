...
For instance, create a file "example1.sjs" with the following content:
...
(define-check (check-eval? env given expected)
  (with-check-info [('simplejs given) ('lambdajs (compile given))]
    (check-equal?
      (s:eval given env)
      expected
    )
